...
function mod:TestVisibility(ignoreToggle)
	...
	if db.visibility.anywhere or
		...
		(db.visibility.battleground and instanceType == "arena") or
...
function mod:COMBAT_LOG_EVENT_UNFILTERED(ev, timestamp, event, hideCaster, sourceGUID, sourceName, sourceFlags, srcFlags2, destGUID, destName, destFlags, dstFlags2, spellID, ...)
...
function mod:CoordsToPosition(x, y)
	...
	if zoneScale[1] == nil then
		zoneScale[1] = 0
	end
	if zoneScale[2] == nil then
		zoneScale[2] = 0
	end
	--print(zoneScale[1], zoneScale[2])
...
function mod:UpdateZoneData()
	...
	--print(max(l,0))
	local _, _, w, h = Astrolabe:GetMapInfo(m, max(l - 1, 0))
	zoneScale[1] = w
	zoneScale[2] = h
