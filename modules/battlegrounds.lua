...
battleground("GilneasBattleground2", "Die Schlacht um Gilneas", {
	options = {
	...
	GetPoiIndexes = function(self)
		wipe(self.pointKeys)
		self:Register("leuchtturm", 6,7,8,9,10,11,12)
		self:Register("minen", 16,17,18,19,20,21,22)
		self:Register("wasserwerke", 26,27,28,29,30,31,32)
	end
})

battleground("TempleofKotmogu", "Der Tempel von Kotmogu", {
	options = {
		kugeldermacht = "Kugel der Macht",
	},
	GetPoiIndexes = function(self)
		wipe(self.pointKeys)
		self:Register("kugeldermacht", 45)
	end
})

battleground("TwinPeaks", "Zwillingsspitze", {
	options = {
		flag = L["Flag Carrier"],
	},
	BattlegroundMessage = function(faction, msg)
...
function mod:UpdateActiveModule()
	...
	if newModule ~= activeModule then
		print("HudMap PVP Module loaded for: ", area);
...
function mod:BattlegroundMessage(event, msg)
	...
	if not activeModule:Option("flag") then
		return
	end
	...
	for i = 1, GetNumMapLandmarks(), 1 do
		...
		if landmark then
			...
			if settings.points[landmark] and x ~= 0 and y ~= 0 and texIndex ~= 0 then
				...
				m.texture:SetTexCoord(GetPOITextureCoords(texIndex))
				POIs[key] = m
				usedPOIs[key] = true
